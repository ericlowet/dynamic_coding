function [ connMat, connMat_sep, R_e, R_i ] = genConnections_eric(numNeur, dR, sig, p, strength)
...
[A_IE]=sampleConn2(pdfConIE,p(IE));
[A_EE]=sampleConn2(pdfConEE,p(EE));
[A_II]=sampleConn2(pdfConII,p(II));
[A_EI]=sampleConn2(pdfConEI,p(EI));
...
function [connMat]=sampleConn2(pdfCon,pCon)

[numReceive,numSend]=size(pdfCon);  
numCon=round(pCon*numSend);
pdfCon(pdfCon <= 0.001 )=NaN;
var_parameter=5;
connMat = round(pdfCon.* (randn(size(pdfCon)).*var_parameter  + numCon)   ) ;
