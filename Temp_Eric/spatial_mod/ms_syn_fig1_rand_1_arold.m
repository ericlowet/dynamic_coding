...
tic
[sigG] = filterdatahilb_bg_edit(signals, 22,65,1000);
toc

xx=exp(1i.*angle(sigG(:,1:1:end))).';
vv=(xx' *xx);
x=round(numNeurLin/2);y=round(numNeurLin/2);
 seed= (x-1)*sqrt(numNeurLin^2)+y;
%%%%%%%%%%%%%%%%%%%%%%
%%% Input diff%%%%
allIdiff=bsxfun(@minus,mean_E(1:Ne1).',mean_E(1:Ne1)); 

%%%%%%%%%%%%%%%%%
consum=bsxfun(@plus,S3(1:Ne1,1:Ne1),S3(1:Ne1,1:Ne1).')/2;
...
[ ms_sigG] = mstrigsig(round(ms_dips./2),sigG); %sampling rate has been halved in the mean time
...
for timsh=1:15:mstim-winti
    ...
    mSP=reshape(ms_sp(:,timsh:timsh+winti,:),Ne1,(winti+1)*ms_n,size(ms_sp,3));
