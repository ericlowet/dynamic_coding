...
basedir='/home/mrphys/bargip/GIT/Dynamic_Coding/Temp_Bart/test_Orient/classification/27';
...
label='LFP';
label='Rate2';
savDir=fullfile(basedir,['TFR_' label '_v2']);
...
tmp=pwd;
if exist('/home/mrphys/bargip/GIT/Dynamic_Coding/Temp_Bart/test_Orient/scripts/TFRClassHandle_v2.mat','file')
  load /home/mrphys/bargip/GIT/Dynamic_Coding/Temp_Bart/test_Orient/scripts/TFRClassHandle_v2.mat
else
  cd /home/mrphys/bargip/GIT/Dynamic_Coding/Temp_Bart/test_Orient/scripts
  batchid='TFRClass_v2';
  compiledfun=qsubcompile(@dumFuncClass_v2,'toolbox',{'stats'},'batchid',batchid);
  save TFRClassHandle_v2 compiledfun
end

cd(tmp)

for winN=1:numWin
  cohDum=permute(coh(:,winN,:,:,:),[1 4 3 5 2]);
  cohDum=cohDum(:,:,:);
   for cnI=1:numConn
    selTrials=connIdx==cnI;
    
    
    if ~exist(fullfile(savDir,['P' num2str(winN,'%02d') '_TFR_' [label '_conn' num2str(cnI)] '.mat']),'file')
      cd ~/GIT/Dynamic_Coding/Temp_Bart/tq
      qsubfeval(compiledfun,cohDum(:,:,selTrials),savDir,stimIdx(selTrials),winN,[label '_conn' num2str(cnI)],'memreq',memrq,'timreq',timrq);
    end
   end
   ...
   for stI=1:numConn
    ...
    if ~exist(fullfile(savDir,['P' num2str(winN,'%02d') '_TFR_' label '.mat']),'file')
      cd ~/GIT/Dynamic_Coding/Temp_Bart/tq
      qsubfeval(compiledfun,cohDum(:,:,selTrials),savDir,connIdx(selTrials),winN,[label '_stim' num2str(stI)],'memreq',memrq,'timreq',timrq);
    end
   end
...
P_conn=P_stim;
tic
P_conn(:)={zeros(numConn)};
P_stim(:)={zeros(numStim)};
for winN=1:numWin
  
  
  for cnI=1:numConn
%     try
    load(fullfile(savDir,['P' num2str(winN,'%02d') '_TFR_' [label '_conn' num2str(cnI)] '.mat']))
    for freqN=1:numFreq
      P_stim{freqN,winN,1}=P_stim{freqN,winN,1}+P_ML{1,freqN};
      P_stim{freqN,winN,2}=P_stim{freqN,winN,1}+P_ML{2,freqN};
    end
%     end
  end
  ...
  for stI=1:numStim
%     try
    load(fullfile(savDir,['P' num2str(winN,'%02d') '_TFR_' [label '_stim' num2str(stI)] '.mat']))
    for freqN=1:numFreq
      P_conn{freqN,winN,1}=P_conn{freqN,winN,1}+P_ML{1,freqN};
      P_conn{freqN,winN,2}=P_conn{freqN,winN,1}+P_ML{2,freqN};
    end
%     end
  end
end

classPerf=reshape(classPerf,[],2);
for n=1:numel(P_conn)
  P_conn{n}=P_conn{n}./sum(P_conn{n}(:));  
  P_stim{n}=P_stim{n}./sum(P_stim{n}(:));
  classPerf(n,1)=sum(diag(P_stim{n}));
  classPerf(n,2)=sum(diag(P_conn{n}));
end
classPerf=reshape(classPerf,[numFreq,numWin,2,2]);
