...
c=nan(numWin,doubleFlag+1);
MI=nan(numWin,doubleFlag+1);
numBinsHist=24;
for winN=1:numWin
  try
...
figure(9)
clf
set(gcf,'position',[150 150 700 300])
subaxis(1,2,1,'mr',.04,'sh',.1,'mb',.16)
...
hold all
ax = gca;
set(ax,'ColorOrder', cols(1:doubleFlag+1,:));
...
hh=legend('Intra (50ms window)','Inter (50ms window)','Intra (100ms window)','Inter (100ms window)');
  ...
  hh=legend('50ms window','100ms window');
end
...
export_fig(fullfile(basedir,'TE_summ'),'-transparent','-pdf','-eps','-png',9)